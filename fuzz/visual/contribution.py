        ...
        for i in range(n_sampling):
            ...
            if save_type == 'img' or self.show_info:
                try:
                    ...
                    pass
            if save_type == 'data':
                c = np.squeeze(guided_grads, axis=0) # delete first dim
                if c.max() == 0:
                ...
                c = c.mean(axis = 0) # 一段时间内平均贡献度

                C.append(c.reshape((1,-1)))
...
class GuidedBackprop(RunData):
    ...
    def guided_bp_hooks(self, first_layer_name):
        ...
        def _forward_act_layer(module, z, h):
            ...
            if self.hook_forward:
                ...
                self.act_h.append((module, z[0].data, h[0].data))
                
        def _backward_act_layer(module, grad_z, grad_h):
            ...
            _grad_h = list(grad_h)[0]
            self.grad_h.append(_grad_h.data)
        ...
        def _backward_linear_layer(module, grad_out, grad_in):
            ...
            with torch.no_grad():
                ...
                (act, z, h) = self.act_h[-1]
                grad_h = self.grad_h[-1]
                
                # print(x.size(), module.weight.size(), grad_h.size())
                # self.hook_forward = False
                c_mat = torch.mm( x.view(-1,1), (1/z).view(1,-1) )
                print(c_mat.size(), module.weight.size(), grad_h.view(-1).diag().size())
                c_mat =  c_mat * module.weight.t() # (d_z, d_x)
                # c_mat = nn.functional.relu(c_mat)
                # c_mat = c_mat/ c_mat.sum(axis = 1).view(-1,1)
                c = torch.mm( c_mat, grad_h.view(-1).diag())
                
                # self.hook_forward = True
                # print(c.size()) 
    ...
    def generate_gradients(self, input_image, target_class):
        ...
        self.act_h = []             # record act module and activation for act layer (forward)
        self.grad_h = []            # record grad_h for act layer (backward)
        # Input (tensor)
