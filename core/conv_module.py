...
class Conv_Module(object):
    ...
    def list2df(self, lst):
        ...
        df = DataFrame( columns = ['Conv', '*', 'Pool', 'Res', 'Loop', 'Out'] )
        times = 1
        cnt = 0
        for i, v in enumerate(lst):
            ...
            # set/res
                ...
                if len(res) == 1: out[3] = res[0]
                elif len(res) > 0: out[3] = res
                df.loc[cnt] = out
                cnt += 1
            times = 1
    ...
    def check_row(self, in_channel, row):
        ...
        conv_para, pool_para, res_para = row[0], row[2], row[3]
        ...
        # pool
        if pool_para != '-':
            row[2] = check_pool(pool_para)
        # res
        process_res = True
        if type(res_para) != str:
            if type(res_para) == int or type(res_para[0]) == int:
                row[3] = check_conv(in_channel, res_para)
                if row[3][1] != out_channel: row[3][1] = out_channel
            else:
                res_dim = in_channel
    ...
    def get_out_size(self, in_size, row):
        ...
        if conv_para != '-': 
            if type(conv_para[0]) == list:
                ...
                for i, para in enumerate(conv_para):
                    cal_para = []
                    for j in range(2, len(para)):
                        if type(para[j]) == int or type(para[j]) == tuple: 
                            cal_para.append(para[j])
                    conv_para[i] = cal_para
            else:
                out_channel = conv_para[1]
                cal_para = []
                for j in range(2, len(conv_para)):
                    if type(conv_para[j]) == int or type(conv_para[j]) == tuple: 
                        cal_para.append(conv_para[j])
                conv_para = cal_para
